...
4. **Reconcile into `tokens.json`.** For each value:
   ...
   - If the design system introduces a genuinely new value used across more than one surface → add a token (per [add-a-token.md](add-a-token.md) — but read its "First, don't" section: snap to an existing token where you can).
